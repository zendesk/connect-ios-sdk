...
class ConfigProvider: BaseProvider {
    ...
    func config(platform: String, version: String, completion: @escaping (Result<Config>) -> Void) {
        WebService(with: client).load(resource: ConfigResource.config(platform: platform, version: version)) { result in
            completion(result)
        }
    }
}
